...
seem that the machine's continuously random edge reassignments would
...
are the previous destination node of both its (multi-) out-edge, resulting in no
change to the edges. An example of such a graph appears in Figure \ref{fig:TerminalRMGraph}.
\begin{marginfigure}
\vspace{-8cm}
...
that \textbf{*M} machine runs collapse: just as \textbf{RM} random operation produces multi-edges,
\textbf{R}'s identical edge rerouting logic typically produces multi-edges in
each iteration's draft output graph that must be pruned out before continuing
execution. \textbf{C}, on the other hand, is more or less likely
to produce prohibited structures, depending on the rule in force for the
run.
...
\begin{table}
\caption{Table \ref{tab:TabQ}: Comparison of Graph Measures, \textbf{C} vs. \textbf{R}}
\vspace{12mm}
\centering
...
\# Avg. Run Length & Approx. twice that of \textbf{R} & \\
Collapse Outcome & & \includegraphics[height=8cm]{figA.png} \\
 & & \\
\addlinespace[2mm]
