...
A summary of outcomes for all runs is given in Table \ref{tab:Tab1}.
...
The absence of multi-edges in machines \textbf{C} and \textbf{R}
...
\textbf{R}'s identical edge rerouting logic
will \textit{always} produce some number of structures that must be pruned out
owing to random variation. \textbf{C}, on the other hand, is more or less likely
to produce prohibited structures, depending on the rule in force for a particular
run.
