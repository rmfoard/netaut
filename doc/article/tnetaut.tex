...
like those in Figure \ref{fig:Multiedges} when operating
under most of \textbf{CM}'s rules. As these structures enter the graph, they 
...
to a configuration in which every node's six possible new out-edge destinations
are all the same node. An example of such a graph appears in Figure \ref{fig:TerminalRMGraph}.
\begin{marginfigure}
\vspace{-8cm}
