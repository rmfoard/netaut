...
If it is not empty, the pruned graph becomes the starting graph for the next iteration.
...
either maintains or reduces the number of nodes.
It is consequently certain that the machine will terminate for every rule,
in either a state cycle or a graph collapse.
The possibility that a graph will traverse an astronomical number
of states\footnote before revisiting a previous configuration, however,
limits simulator's ability to detect a cycle.
\[
