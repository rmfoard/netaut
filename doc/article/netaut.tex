...
selected at random from a large universe of possibilities. Measurements were recorded as graphs
...
These conditions are established when initial graphs are generated.
Both machine types maintain them throughout execution. In
Machine \textbf{C}, a "pruning" operation is performed at the end of each
iteration if necessary to eliminate prohibited structures; the process
is described in a later section. %BOOKMARK%

A second machine, \textbf{CM}, was also implemented. It is identical with
machine \textbf{C} but allows multiple, like-directed edges between nodes
and consequently does not need to perform pruning.
...
constraint that both edges cannot have the same destination node is observed.
In all cases, nodes have out-degree 2, but in-degree can vary from 0 to N.
Each node is assigned a \textit{black} or \textit{white} state at random,
...
Machine \textbf{CM} uses the same rule structures. It operates exactly as
machine \textbf{C} does, but cannot create prohibited structure and consequently
does not incorporate a pruning process or produce "collapse" outcomes. 
...
increase order as they transform random starting graphs, simulators for counterpart machines \textbf{R} and \textbf{RM}
were constructed. Like the rule-based \textbf{C*} machines, these change graphs
iteratively, but make their changes \textit{randomly}.
As with the \textbf{C*} machines, though, the scope of changes at each node is
limited to the destinations of its two out-edges. (They need not alter nodes' states
...
The draft copy is restored to conformance with the structure invariant by applying a
pruning process. In the diagram (Figure \ref{fig:Fig2}),
the two illegal configurations that require pruning are shown as cases I and II.
...
Redirected edges are shown in red.
...
\begin{figure*}[tb] 
    ...
    \caption{\textbf{Pruning.} Resolution methods for two variations of a prohibited structure.
In both cases, edges originating at node A are eliminated; edges in red are redirected.
...
before its machine terminates, the outcome is recorded as "undetermined."
Aggregate outcome statistics for all simulations appear in Table \ref{tab:Tab1}.
