...
For machine \textbf{CM}, the last restriction is omitted.
...
machine \textbf{C} but removes the restriction on multiple, like-directed
edges between nodes.
...
To help gauge the extent to which machine \textbf{C} increases order as it
transforms random starting graphs, a simulator for a second machine, \textbf{R},
...
instead of under the control of a rule. As with \textbf{C}, though, the scope
of \textbf{R}'s changes at a node is limited to the node's state and the destinations of
its two out-edges.

\subsection{Pruning}

Changes to out-edge destinations, whether made randomly or based on \textbf{C}'s
...
pruning process. In the diagram (Fig. \ref{fig:Fig2}),
...
case I resolutions may or may not. The cascading reductions in graph size are
common for both \textbf{C} and \textbf{R} machines.
...
It is consequently certain that machine runs will terminate,
