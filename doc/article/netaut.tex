...
edges between pairs of nodes. These prohibited structures can arise in the
draft graphs created during an iteration's first pass, and must be removed
to restore the structure invariant before leaving the iteration.
Two kinds of situations must be resolved before the machine can begin its
next iteration (Fig. \ref{fig:Fig2}).

\begin{figure*}[tb] 
    \centering
    \makebox[\textwidth]{\includegraphics[width=.9\paperwidth]{./pruning.png}}
    \caption{Resolution methods for two variations of a prohibited structure.
